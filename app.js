// CFD Anything - viewer of the precomputed flow_v02 predictions in data/ (written by website/pipeline/predict.py, whose
// pack_frame / pack_vel / write_mesh document the binary layouts). Frames exist for discrete (yaw, pitch, Re): the body
// is tilted nose up by pitch about y, then turned by yaw about z; in flow axes the flow always comes along +x. On top,
// the body can be rolled about the flow axis x: in free stream that turns the whole solution with it, so the shown
// frames are drawn rolled (body and lines together) and nothing new is needed; the velocity plane is the plane through
// x that faces the camera best (each frame stores planes through x every 15 deg of roll).
// Two rotations: the drag on the view is a trackball that turns the whole scene (state.view: body and flow together,
// nothing recomputed); the dial top left turns the flow around the body (state.q), which stays put on screen.
...
// URL: ?shape=cow&yaw=35&pitch=20&roll=45&view=0.1,0.2,0,0.97&re=500000&mode=velocity&lines=0&style=engineering&smooth=0
...
// view: flow axes -> screen scene (the drag on the view); the body on screen is view q.
// dragMode: what a drag on the view turns, "flow" (around the body, which stays put) or "view" (body and flow together)
// Defaults (also for every newly selected shape): drag turns the view, pressure shown
const DEFAULT_DRAG = "view", DEFAULT_FIELD = "pressure";
const state = { shapes: [], current: null, lines: true, field: DEFAULT_FIELD, style: "paper", re: 5e5, smooth: true,
                view: new THREE.Quaternion(), dragMode: DEFAULT_DRAG,
...
const AZIMUTH = -90 * DEG, TILT = 16 * DEG;                 // fixed camera; at view = identity across the flow, left to right
...
flowGroup.add(lines);
const viewGroup = new THREE.Group();                         // everything in flow axes, turned by state.view
viewGroup.add(flowGroup, velPlane);                          // the plane faces the camera (see rollVel)
scene.add(viewGroup);
let bodyObj = null;
let dashTime = 0;
...
function placeCamera() {
  ...
  target.copy(TARGET).multiplyScalar(t).applyQuaternion(state.view);    // behind the body along the flow
...
// the view drag: turn the scene (body and flow together) to q
function setView(q) {
  state.view.copy(q).normalize();
  viewGroup.quaternion.copy(state.view);
  placeCamera();
  drawTriad();
  drawDial();
...
// the flow direction dial (#dial): the free stream in camera axes (x right, y up, z towards the viewer), drawn as three
// streams with moving dashes and a head, the knob at the head; seen end-on a dot (towards the viewer) or a cross (away)
const DIAL_R = 30;                                           // sphere radius in the svg's units (viewBox 88 wide)
const dial = $("#dial"), dialStreams = [...dial.querySelectorAll(".stream")];
const dialFlow = () => X_AXIS.clone().applyQuaternion(state.view).applyQuaternion(camera.quaternion.clone().invert());

function drawDial() {
  const d = dialFlow(), l = Math.hypot(d.x, d.y);
  const ux = l > 1e-6 ? d.x / l : 1, uy = l > 1e-6 ? -d.y / l : 0, px = -uy, py = ux, len = DIAL_R * l;
  const seg = (o, a, b) => `M${(o * px + a * ux).toFixed(2)} ${(o * py + a * uy).toFixed(2)}L${(o * px + b * ux).toFixed(2)} ${(o * py + b * uy).toFixed(2)}`;
  dialStreams.forEach((el, i) => {
    const o = (i - 1) * 13, s = i === 1 ? 1 : 0.75;
    el.setAttribute("d", len > 2 ? seg(o, -len * s, len * s - (i === 1 ? 3 : 0)) : "");
    el.style.opacity = String(0.3 + 0.7 * smoothstep(0.15, 0.45, l));
  });
  const tx = ux * len, ty = uy * len;
  dial.querySelector(".head").setAttribute("d", len > 8
    ? `M${tx - 8 * ux + 5 * px} ${ty - 8 * uy + 5 * py}L${tx - 2 * ux} ${ty - 2 * uy}L${tx - 8 * ux - 5 * px} ${ty - 8 * uy - 5 * py}` : "");
  const sym = dial.querySelector(".sym");
  sym.style.opacity = String(1 - smoothstep(0.2, 0.5, l));
  sym.querySelector(".dot").style.display = d.z > 0 ? "" : "none";
  sym.querySelector(".cross").style.display = d.z > 0 ? "none" : "";
  const knob = dial.querySelector(".knob");
  knob.setAttribute("cx", tx.toFixed(2));
  knob.setAttribute("cy", ty.toFixed(2));
  knob.style.opacity = d.z < -0.05 ? "0.5" : "1";            // behind the dial's sphere
  showReset();
}

// "reset" under the dial, while the flow or the view is turned away from the start
const atStart = (q) => Math.abs(q.w) > 0.99995;
const showReset = () => { $("#reset").hidden = atStart(state.view) && atStart(state.qTo); };

function drawTriad() {
  const inv = camera.quaternion.clone().invert();
  const axes = [["x", new THREE.Vector3(1, 0, 0)], ["y", new THREE.Vector3(0, 1, 0)], ["z", new THREE.Vector3(0, 0, 1)]]
    .map(([name, v]) => [name, v.applyQuaternion(state.view).applyQuaternion(inv)])
...
// The plane through the flow axis facing the camera. Plane k of a frame's n holds its flow at theta = k 180 / n through
// the x axis, (x, s sin theta, s cos theta); drawn rolled by phi about x, plane theta = phi + alpha is the flow-axes
// plane spanned by x and (0, sin alpha, cos alpha), s along the latter (past 180 deg: plane - 180 with s flipped).
// alpha turns that plane's normal onto the view direction (in flow axes, across x). Between two planes it blends them,
// texel by texel. Files with only the plane y = 0 (built before 2026-09-11) roll with the flow instead, fading edge-on.
function rollVel() {
  const [v0] = velParts[0], nx = v0.nx, nz = v0.nz;
  velRolled = velParts.every(([v]) => v.planes > 1);
  const view = camera.getWorldDirection(new THREE.Vector3()).applyQuaternion(state.view.clone().invert());
  const across = Math.hypot(view.y, view.z), alpha = across > 1e-6 ? Math.atan2(-view.z, view.y) / DEG : 0;
  const phi = velRolled ? norm360(state.flowRoll + alpha) : 0;
  velAcc.fill(0);
  let ws = 0;
  ...
  const roll = (velRolled ? -alpha : state.flowRoll) * DEG, s = velSlice;
  velPlane.rotation.x = Math.PI / 2 + roll;                  // x-z plane turned about x (legacy files: with the flow)
  velPlane.position.set((s.x[0] + s.x[1]) / 2, s.y, (s.z[0] + s.z[1]) / 2).applyAxisAngle(X_AXIS, roll);
  // plane normal: y turned about x; looking along the flow it would only be a smear
  velMaterial.uniforms.uFacing.value = velRolled ? across : Math.abs(Math.cos(roll) * view.y + Math.sin(roll) * view.z);
}
...
// ± next to drag and lift: mean error of flow_v02 on the 111 val cases of v02_fullre (43 shapes).
// Cd: relative (5.2 %). Cl: relative over the 44 cases with |Cl| >= 0.2 (14 %), at least the mean absolute error of all
// 111 (0.057), since lift is often ~0 and a relative error means nothing there
const CD_REL_ERR = 0.052, CL_REL_ERR = 0.14, CL_ABS_ERR = 0.057;
...
// Share of the flow directions computed so far. The local preview writes meta.total; the online service (website/inference)
// computes UPLOAD_ANGLES level directions (config.YAWS) and writes total only in newer versions.
const UPLOAD_ANGLES = 12;
const uploadFailed = (entry) => ["failed", "interrupted"].includes(entry.meta?.status);
function uploadProgress(entry) {
  const m = entry.meta;
  if (!m?.frames) return 0;
  return Math.min(1, m.frames.length / (m.total > 0 ? m.total : UPLOAD_ANGLES));
}

// After the name: a ring filling up (with a small dot running round) while the directions are computed, then the dot
// until the shape is opened
function markUpload(entry) {
  if (entry.meta?.status === "done" && state.current === entry) markSeen(entry);   // finished while being looked at
  const row = document.querySelector(`.shape[data-name="${entry.name}"]`);
  if (!row) return;
  row.querySelector(".dot").hidden = !showDot(entry);
  const ring = row.querySelector(".ring"), pct = Math.round(100 * uploadProgress(entry));
  ring.hidden = !isPending(entry) || uploadFailed(entry);
  ring.querySelector(".arc").setAttribute("stroke-dasharray", `${pct} 100`);
  ring.title = ringTitle(entry);
}

function ringTitle(entry) {
  return entry.sending ? "uploading your shape…" : `computing all flow directions ${Math.round(100 * uploadProgress(entry))}%`;
}
...
async function watchUpload(entry) {
  if (entry.watching || entry.sending) return;               // still being sent: nothing on the service yet
  entry.watching = true;
  if (!entry.meta) {
    try {
  ...
  while (isPending(entry) && entry.meta?.status !== "failed") {
    ...
    markUpload(entry);                                       // the ring in the list, selected or not
...
function showUploadState(entry) {
  ...
  $("#upload-progress").hidden = !pending || failed;
  $("#upload-progress").textContent = `computing all flow directions ${Math.min(99, Math.floor(100 * uploadProgress(entry)))}%`;
...
async function selectShape(name) {
  ...
  const switching = !!state.current && state.current !== entry;   // not the first shape of the page (URL orientation)
  state.current = entry;
  if (switching) {                                           // another shape starts from the defaults
    state.dragMode = DEFAULT_DRAG;
  ...
  if (bodyObj) {
    viewGroup.remove(bodyObj.group);
    disposeBody(bodyObj);
  }
  bodyObj = makeBody(entry, STYLES[state.style]);
  viewGroup.add(bodyObj.group);
  velMaterial.uniforms.uMax.value = entry.meta.speed_max ?? 2;
  // another shape starts at the original orientation (level body, flow left to right), and so does an upload still
  // computing, where its first frame is; the first shape of the page keeps the orientation from the URL
  setTarget(switching || isPending(entry) ? new THREE.Quaternion() : state.q);
  setShown(state.qTo);
  if (switching) setView(new THREE.Quaternion());
...
function buildList(shapes) {
  ...
  for (const s of shapes) {
    ...
    let label = s.label;
    if (s.upload) {                                          // ring / dot stay on the line of the last word
      const dot = `<span class="dot" title="all flow directions computed"${showDot(s) ? "" : " hidden"}></span>`;
      const pct = Math.round(100 * uploadProgress(s));
      const fail = uploadFailed(s) ? `<small class="fail">failed</small>` : "";
      const ring = `<span class="ring" title="${ringTitle(s)}"${isPending(s) && !uploadFailed(s) ? "" : " hidden"}>` +
        `<svg viewBox="0 0 16 16" aria-hidden="true"><circle class="track" cx="8" cy="8" r="6"/>` +
        `<circle class="arc" cx="8" cy="8" r="6" pathLength="100" stroke-dasharray="${pct} 100"/>` +
        `<g class="orbit"><circle cx="8" cy="2" r="1.4"/></g></svg></span>`;
      const cut = s.label.lastIndexOf(" ") + 1;
      label = `${s.label.slice(0, cut)}<span class="nowrap">${s.label.slice(cut)}${ring}${dot}${fail}</span>`;
    }
    ...
    if (s.error) b.title = `could not compute this shape: ${s.error}`;
    b.addEventListener("click", () => {
      if (s.sending || (s.upload && !s.base)) return;        // still uploading, or the upload failed: nothing to show
      track(`shape/${s.name}`);
      selectShape(s.name);
    });
    list.append(b);
...
function saveUrl() {
  ...
  urlTimer = setTimeout(() => {
    ...
    const v = state.view;
    if (Math.abs(v.w) < 0.99999) p.set("view", [v.x, v.y, v.z, v.w].map((c) => c.toFixed(4)).join(","));
    if (state.reShown !== state.current.meta.re) p.set("re", String(state.reShown));
    if (state.field !== DEFAULT_FIELD) p.set("mode", state.field ?? "none");
    if (!state.lines) p.set("lines", "0");
    if (!state.smooth) p.set("smooth", "0");
    if (state.dragMode !== DEFAULT_DRAG) p.set("drag", state.dragMode);    if (Math.abs(zoomTo - 1) > 0.005) p.set("zoom", zoomTo.toFixed(2));
...
canvas.addEventListener("pointermove", (ev) => {
  ...
  const flow = state.dragMode === "flow";
  if (!drag || !state.current?.meta || (flow && locked())) return;
  ...
  if (s > 1e-9) (flow ? turnFlow : turnView)(axis.divideScalar(s), Math.atan2(s, a.dot(b)) * arc.r * DRAG_SPEED);
});

// The drag is a virtual trackball around the body (Holroyd's sphere, a hyperbola outside it) that turns the flow
// around the body or the whole scene ("drag turns: flow · view"): through the middle, sideways turns about the vertical
// axis and up / down about the horizontal one; out at the sides the drag twists about the view axis. The axes are the
// screen scene's (x right, z up, -y towards the viewer), not the slightly tilted camera's, so sideways stays a pure
...
const hideHint = () => $("#hint").classList.add("gone");    // "drag to turn…" goes with the first turn

function turnView(axis, deg) {                               // turn body and flow together about a screen-scene axis
  hideHint();
  setView(axisQuat(axis, deg).multiply(state.view));
  saveUrl();
}

// Turn the flow by deg about a screen-scene axis while the body stays where it is on screen: in flow axes that is the
// body turned the other way, C = view^-1 R^-1 view. The view follows the body, so whatever setTarget allows (the
// computed tilt range) the body does not move; without smooth turning it then glides to the closest computed frame.
function turnFlow(axis, deg) {
  hideHint();
  const inv = state.view.clone().invert(), shown = state.view.clone().multiply(state.q);
  const C = inv.multiply(axisQuat(axis, -deg)).multiply(state.view);
  setTarget(C.clone().multiply(state.qTo));
  setShown(state.smooth ? state.qTo : C.multiply(state.q));
  setView(shown.multiply(state.q.clone().invert()));
  requestBlend();
}

$("#reset").addEventListener("click", () => {               // back to the start: level body, flow left to right
  track("view/reset");
  setTarget(new THREE.Quaternion());
  setShown(state.qTo);
  setView(new THREE.Quaternion());
  requestBlend();
  saveUrl();
});
...
const endDrag = () => {
  ...
  if (state.dragMode === "flow" && !state.smooth) snapTarget();   // the next turn starts from the frame on screen
  saveUrl();
};

// The dial: a trackball of the dial's own size in camera axes, so the knob stays under the pointer. A knob behind the
// sphere (flow away from the viewer) is grabbed on the back half.
let dialDrag = null;

function dialPoint(x, y, back) {
  const r = dial.querySelector("svg").getBoundingClientRect(), R = (r.width * DIAL_R) / 88;
  const px = (x - r.left - r.width / 2) / R, py = (r.top + r.height / 2 - y) / R, d2 = px * px + py * py;
  return new THREE.Vector3(px, py, (back ? -1 : 1) * (d2 <= 0.5 ? Math.sqrt(1 - d2) : 0.5 / Math.sqrt(d2))).normalize();
}

dial.addEventListener("pointerdown", (ev) => {
  if (!state.current?.meta || locked()) return;
  ev.preventDefault();
  dial.setPointerCapture(ev.pointerId);
  const d = dialFlow(), p = dialPoint(ev.clientX, ev.clientY, false);
  dialDrag = { x: ev.clientX, y: ev.clientY, back: d.z < 0 && Math.hypot(p.x - d.x, p.y - d.y) < 0.5 };
  dial.classList.add("dragging");
  once("flow");
});
dial.addEventListener("pointermove", (ev) => {
  if (!dialDrag || locked()) return;
  const a = dialPoint(dialDrag.x, dialDrag.y, dialDrag.back), b = dialPoint(ev.clientX, ev.clientY, dialDrag.back);
  dialDrag.x = ev.clientX;
  dialDrag.y = ev.clientY;
  const axis = new THREE.Vector3().crossVectors(a, b), s = axis.length();
  if (s > 1e-9) turnFlow(axis.divideScalar(s).applyQuaternion(camera.quaternion), Math.atan2(s, a.dot(b)) / DEG);
});
const endDial = () => {
  if (!dialDrag) return;
  dialDrag = null;
  dial.classList.remove("dragging");
  if (!state.smooth) snapTarget();                           // the next turn starts from the frame on screen
  saveUrl();
};
dial.addEventListener("pointerup", endDial);
dial.addEventListener("pointercancel", endDial);
// arrow keys on the dial: left / right turn the flow about the vertical, up / down tilt it
dial.addEventListener("keydown", (ev) => {
  ...
  if (!state.current?.meta || (!side && !up) || locked()) return;
  ev.preventDefault();
  if (side) turnFlow(Z_AXIS, 10 * side);
  else turnFlow(Y_AXIS, -10 * up);
  if (!state.smooth) snapTarget();
  saveUrl();
});
const endPointer = (ev) => {
...
// arrow keys turn like the drag through the middle (flow or view): left / right about the vertical axis, up / down
...
canvas.addEventListener("keydown", (ev) => {
  ...
  const up = ev.key === "ArrowUp" ? 1 : ev.key === "ArrowDown" ? -1 : 0;
  const flow = state.dragMode === "flow";
  if (!state.current?.meta || (!side && !up) || (flow && locked())) return;
  ev.preventDefault();
  if (!flow) return turnView(side ? Z_AXIS : X_AXIS, side ? 10 * side : -10 * up);
  turnFlow(side ? Z_AXIS : X_AXIS, side ? 10 * side : -10 * up);
  if (!state.smooth) snapTarget();
  saveUrl();
});

const DRAG_TEXT = {
  flow: ["drag to turn the flow around the object", "Drag or use the arrow keys to turn the flow around the shape."],
  view: ["drag to turn the object and the flow together", "Drag or use the arrow keys to turn the view, shape and flow together."],
};

function applyDragMode() {
  for (const b of document.querySelectorAll(".drag-mode [data-drag]")) b.setAttribute("aria-pressed", String(b.dataset.drag === state.dragMode));
  const [hint, label] = DRAG_TEXT[state.dragMode];
  $("#hint").textContent = hint;
  canvas.setAttribute("aria-label", `3D view of the flow around the selected shape. ${label} Scroll, pinch or use plus and minus to zoom.`);
}

for (const b of document.querySelectorAll(".drag-mode [data-drag]")) {
  b.addEventListener("click", () => {
    state.dragMode = b.dataset.drag;
    track(`view/drag-${state.dragMode}`);
    applyDragMode();
    saveUrl();
  });
}
...
async function main() {
  ...
  // account.js sends uploads in the background: listed at once (ring), swapped for the service's entry when it took the
  // file, not opened automatically (the visitor may be looking at something else)
  window.addEventListener("cfd:upload-start", (ev) => {
    state.shapes.push({ name: ev.detail.name, label: ev.detail.label, upload: true, sending: true, meta: { status: "sending", frames: [] } });
    buildList(state.shapes);
    drawThumbnails(state.shapes);
    document.querySelector(`.shape[data-name="${ev.detail.name}"]`)?.scrollIntoView({ block: "nearest" });
  });
  window.addEventListener("cfd:upload-failed", (ev) => {
    const entry = state.shapes.find((s) => s.name === ev.detail.name);
    if (!entry) return;
    entry.meta.status = "failed";
    entry.error = ev.detail.error;
    buildList(state.shapes);
    drawThumbnails(state.shapes);
  });
  window.addEventListener("cfd:upload", (ev) => {
    const d = ev.detail, i = state.shapes.findIndex((s) => s.name === d.replaces);
    if (i >= 0) state.shapes[i] = { name: d.name, label: d.label, base: d.base, upload: true };   // keeps its place
    addUploads(i >= 0 ? [] : [d]);
  });
  buildList(state.shapes);
  for (const s of state.shapes) if (s.upload) watchUpload(s);
  ...
  if (v.length === 4 && v.every(Number.isFinite) && Math.hypot(...v) > 0.5) setView(new THREE.Quaternion(...v));
  if (speedEl && Number(p.get("re")) > 0) state.re = Number(p.get("re"));
  if (FIELDS.includes(p.get("mode"))) state.field = p.get("mode");
  else if (p.get("mode") === "none") state.field = null;
  if (p.get("lines") === "0") state.lines = false;
  state.smooth = p.get("smooth") !== "0";
  if (["flow", "view"].includes(p.get("drag"))) state.dragMode = p.get("drag");
  applyDragMode();
